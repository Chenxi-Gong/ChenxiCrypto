...
New feature:
Change module name FetchData to CryptoData;
The Simulation module can be applied to any stock market.
Last updated at 2022/02/26.
'''
...
class Simulation:
    ...
    def simulate(self):
        if self.startAt == None:
            print('missing define: self.startAt')
            return
        if self.endAt == None:
            print('missing define: self.endAt')
            return
        if self.timeList == None:
            print('missing define: self.timeList')
            return
            #self.timeList = self.data.timeStamp(self.startAt, self.endAt, self.ttype)
        if self.priceList == None:
            print('missing define: self.priceList')
            return
